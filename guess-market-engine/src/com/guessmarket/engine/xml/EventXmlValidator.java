...
final class EventXmlValidator {
    ...
    static Event validate(EventXml xml, Set<String> usedNames) throws XmlValidationException {
        String name = requireText(xml.getName(), "Event is missing its 'name' attribute.");

        if (!usedNames.add(name.toLowerCase())) {
            throw new XmlValidationException("Duplicate event name: " + name + ". An event named '" + name + "' already exists.");
        }

        String description = requireText(xml.getDescription(),
                "Event " + name + " is missing a <description>.");

        Commission commission = validateCommission(xml, name);
        List<Option> options = validateOptions(xml, name);
        TradingMethod method = validateMethod(xml, name);
    ...
    private static Commission validateCommission(EventXml xml, String name) throws XmlValidationException {
        if (xml.getCommission() == null || xml.getCommission().getValue() == null) {
            throw new XmlValidationException("Event ''" + name + "'': missing <commission>.");
        }
        int percentage = xml.getCommission().getValue();
        if (percentage < 0 || percentage > 90) {
            throw new XmlValidationException(
                    "Event ''" + name + "'': commission must be between 0 and 90, got: " + percentage);
        }
        CommissionType type;
        try {
            type = CommissionType.fromXmlString(xml.getCommission().getType());
        } catch (IllegalArgumentException e) {
            throw new XmlValidationException("Event ''" + name + "'': " + e.getMessage());
        }
        return new Commission(percentage, type);
    }

    private static List<Option> validateOptions(EventXml xml, String name) throws XmlValidationException {
        if (xml.getOptions() == null || xml.getOptions().size() != 2) {
            throw new XmlValidationException("Event ''" + name + "'': must contain exactly 2 <GM-option> entries.");
        }
        List<Option> options = new ArrayList<>();
        for (String optionName : xml.getOptions()) {
            if (optionName == null || optionName.isBlank()) {
                throw new XmlValidationException("Event ''" + name + "'': option name cannot be empty.");
            }
    ...
    private static TradingMethod validateMethod(EventXml xml, String name) throws XmlValidationException {
        if (xml.getMethod() == null) {
            throw new XmlValidationException("Event ''" + name + "'': missing <GM-method>.");
        }
        TradingMethod method = switch (xml.getMethod().getType()) {
            case LMSR -> {
                LmsrXml lmsr = xml.getMethod().getLmsr();
                if (lmsr.getB() == null) {
                    throw new XmlValidationException("Event ''" + name + "'': missing LMSR <b>.");
                }
            ...
            case ORDERBOOK -> {
                ...
                if (ob.getD() == null || ob.getInitial() == null) {
                    throw new XmlValidationException("Event ''" + name + "'': missing Order Book <d> / <initial>.");
                }
                yield new OrderBookMethod(ob.getD(), ob.getInitial(), ob.isAllowMint());
            }
            case NONE -> throw new XmlValidationException(
                    "Event ''" + name + "'': <GM-method> must contain <GM-LMSR> or <GM-order-book>.");
            case NOTDEFINED -> throw new XmlValidationException(
                    "Event ''" + name + "'': <GM-method> defines more than one trading method.");
        };
        try {
            method.validate();
