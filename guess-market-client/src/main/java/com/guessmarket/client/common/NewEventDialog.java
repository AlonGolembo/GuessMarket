package com.guessmarket.client.common;
...
/**
 * Modal "Add Event" form. Collects a trading method and the event's details
 * (the method-specific fields swap when the method changes), does light
 * client-side validation, and returns a {@link NewEventDTO} when the user
 * clicks Create - or an empty {@link Optional} on Cancel. The market maker is
 * always the caller - this client represents exactly one logged-in identity,
...
public final class NewEventDialog {
    ...
    public static Optional<NewEventDTO> show(Window owner, String marketMakerName) {
        Dialog<NewEventDTO> dialog = new Dialog<>();
        dialog.setTitle("Add Event");
        dialog.setHeaderText("Create a new event - you will be its market maker");
        ...
        // --- Controls --------------------------------------------------------
        Label marketMakerLabel = new Label(marketMakerName);
        ...
        int r = 0;
        addRow(grid, r++, "Market maker", marketMakerLabel);
        ...
        createButton.addEventFilter(ActionEvent.ACTION, evt -> {
            String problem = firstProblem(methodCombo, nameField, descriptionField,
    ...
    private static String firstProblem(ComboBox<TradingMethodType> method,
